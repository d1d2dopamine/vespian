 ...
 *
 * Provided once by [dev.lattice.ui.LatticeTheme] so no primitive invents its own
 * accessibility policy, and so an in-flight transition snaps to its destination
 * the moment motion is turned off.
 */
public val LocalLatMotionEnabled = staticCompositionLocalOf { true }
 ...
 */
@Composable
public fun <T> latControlSpec(
    durationMillis: Int = Motion.controlChangeDurationMillis,
): FiniteAnimationSpec<T> = if (LocalLatMotionEnabled.current) {
