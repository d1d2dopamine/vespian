...
public fun dangerFor(palette: LatPalette): Color {
 ...
 * grey that works on near-black is a smudge on paper. [nameKey] is a key in the
 * consuming app's own string catalogue, so palette names are translated like
 * everything else instead of being hardcoded English inside a library.
 */
@Immutable
public data class LatPaletteSpec(
    val id: String,
    val nameKey: String,
    val dark: LatPalette,
    val light: LatPalette,
) {
    public fun palette(light: Boolean): LatPalette = if (light) this.light else this.dark
}
 ...
 */
public val LatPalettes: List<LatPaletteSpec> = listOf(
...
public const val DEFAULT_LAT_PALETTE_ID: String = "ink"

/** The palette behind a stored id. Anything unknown resolves to the default. */
public fun latPaletteSpec(id: String): LatPaletteSpec =
    LatPalettes.firstOrNull { it.id == id }
        ?: LatPalettes.first { it.id == DEFAULT_LAT_PALETTE_ID }

public val DefaultLatPaletteSpec: LatPaletteSpec get() = latPaletteSpec(DEFAULT_LAT_PALETTE_ID)

public val LatDarkPalette: LatPalette get() = DefaultLatPaletteSpec.dark

public val LatLightPalette: LatPalette get() = DefaultLatPaletteSpec.light
 ...
 */
public val LatTints: List<Color> = listOf(
...
/** No tint chosen. */
public const val LAT_NO_TINT: Int = -1
 ...
 */
public fun latTintColor(index: Int, fallback: Color): Color =
    if (index == LAT_NO_TINT || index !in LatTints.indices) fallback else LatTints[index]
